...
void search_label_downwards(int fd, char temp_char, char label_to_find[], labelsT *labels){
  int i, bytes_read;
  char input_buffer[STANDARD_SIZE];
...
int read_varval(int fd, varT *locals,char *temp_char, char command[]){
  int varval;char buffer[STANDARD_SIZE];
...
void print_command(int fd, varT *locals, char *temp_char){
  char varval_input[STANDARD_SIZE];
  ssize_t bytes_read;
  int varval1;
...
void load_command(int fd, varT *locals, varT *globals, char *temp_char){
  char var_op[STANDARD_SIZE], input_buffer[STANDARD_SIZE];
  int varval1;
...
int main_program(programT *program, programT *programs, pthread_mutex_t *program_mtx){
  ...
  char input_buffer[STANDARD_SIZE] = "";
  char var_op[STANDARD_SIZE], varval1_op[STANDARD_SIZE];
  char temp_char, label[STANDARD_SIZE] ="", command[COMMAND_SIZE]="";
  off_t temp_offset;
  varT* current;
  ...
  if(command_group == 10){ // RETURN
    ...
    if(current == globals){
      fprintf(stderr, "Something went terribly wrong. This should never appear\n");
      mtx_unlock(&glob_mtx, __LINE__);
      exit(1);
    }
    mtx_unlock(&glob_mtx, __LINE__);
